      ...
      <div className="flex  flex-grow  flex-col overflow-y-auto text-5xl ">
        ...
        {domainIsAvailable && (
          <DomainUserInfoForm
            initialData={
              {
                registrantCity: userPurchaseDetails?.city ?? "",
                registrantCountry: userPurchaseDetails?.country ?? "",
                registrantAddress1: userPurchaseDetails?.address ?? "",
                registrantLastName: userPurchaseDetails?.last_name ?? "",
                registrantFirstName: userPurchaseDetails?.first_name ?? "",
                registrantPostalCode: userPurchaseDetails?.postal_code ?? "",
                registrantPhoneNumber: userPurchaseDetails?.phone_number ?? "",
                registrantEmailAddress:
                  userPurchaseDetails?.email_address ?? "",
                registrantStateProvince:
                  userPurchaseDetails?.state_province ?? "",
                registrantPhoneCountryCode:
                  userPurchaseDetails?.phone_code ?? "",
              }
              // {
